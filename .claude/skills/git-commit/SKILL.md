...
6. **Sanity-check sensitive content.** Before drafting, scan the final commit set (the staged diff; in batched mode, the union of every batch's paths diffed against HEAD, plus the full content of untracked paths read directly, since `git diff HEAD` does not show them; on an unborn branch, where `HEAD` does not resolve and `git diff HEAD` is fatal, use `git diff --cached -- <paths>` for staged paths and direct reads for the rest) on two levels:
