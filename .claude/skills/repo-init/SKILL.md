...
6. **Optional scaffolding.** Two layers, mirroring `hf-setup`: 6a picks WHAT to scaffold (design), 6b to 6f each render their exact content or command and gate it (execution). Skip 6a entirely when the user already named what they want in the current conversation.
   ...
   e. **Origin remote (only when absent).** Take the URL from the user (ask as free text if not already given), render `git remote add origin <url>`, then ask **add** / **abort**. Both transports are supported. When the user has no URL-form preference, probe both readiness signals in parallel: `git config --get credential.helper` (a `manager` hit means HTTPS authenticates through the credential manager) and `ssh -o BatchMode=yes -o ConnectTimeout=5 -T git@github.com` (a `successfully authenticated` greeting means SSH is ready; the probe fails fast without prompting). Recommend whichever is ready; when both are, let the user pick; when neither is, HTTPS is the lower-setup path on Windows, and GitHub Desktop's Add-local-repository + Publish wires an HTTPS origin automatically. Verify with `git remote -v` and include it in the step-9 report. Adding the remote never pushes anything; publishing stays with `git-push`.
