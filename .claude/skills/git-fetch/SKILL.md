...
2. **Fetch.** Run exactly:
   ...
   ```
   `--prune` drops local `refs/remotes/origin/*` entries whose remote counterparts are gone; `--tags` pulls release tags. Capture stderr: it lists new branches and pulled tags inline (`* [new branch]`, `* [new tag]`) and the step-5 report consumes that capture directly. Route a non-zero exit: `'origin' does not appear to be a git repository` means the repo has no origin remote, so suggest `git remote add origin <url>` (user-run; recommend the HTTPS URL form when `git config --get credential.helper` reports a manager, since SSH needs separate key and known_hosts setup) and STOP; anything else (network, auth, proxy), surface stderr verbatim and STOP. No retry, no credential helper, no fallback.
