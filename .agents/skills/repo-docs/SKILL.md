...
* Only the About panel text changes: edit `.github/settings.yml` directly, then apply it on GitHub by hand.
...
1. **Probe, read-only.** Run the probe below as written, under `sh`, so a glob that matches nothing is safe in zsh too; then read the `Project` and `Commands` sections of `AGENTS.md`.
   ...
   echo "== settings record"; ls .github/settings.yml 2>/dev/null
   echo "== template residue"
   grep -lE 'tmpl\.general-repo|General Repository Template' README* CONTRIBUTING* CODE_OF_CONDUCT* SECURITY* .github/*.md docs/*.md docs/i18n/*.md .github/assets/* .github/settings.yml 2>/dev/null
...
2. **Pick the scope** with two multi-select choice questions asked together, preselecting what is missing or is template residue: "documents": **README**, **CONTRIBUTING**, **CODE_OF_CONDUCT**, **SECURITY**; "legal": **LICENSE**, **TERMS** (hosted services only). Nothing selected ends the skill.
   ...
   * **Settings record.** `.github/settings.yml` is in scope whenever README is, since its description mirrors the README tagline; it takes a mode like any other file, and the user may drop it from the step-9 plan. It is GitHub-specific: when the origin is on another forge, it is out of scope and a residue copy is listed for deletion.
...
11. **Write.** On **write all**, follow the plan in this order, so no write lands on a file before that file has been moved or saved:
    ...
    3. Write the English files and the settings record, and move into place each fetched text the plan lists (only those); then write the translations and switcher changes.
...
12. **Report.** List the files written, moved, and deleted, each file's mode, the license, the badges shown and reserved, and every placeholder left (logo file, missing facts). On GitHub, when `SECURITY.md` points at private vulnerability reporting, remind the owner to enable it (Settings, Security). When the settings record was written, remind the owner that GitHub does not read it: compare the About panel with the file first, so a value set only on the web is not lost, then apply it by hand (the gear icon next to About, and Settings, General for the template flag) unless the Settings App is installed; when GitHub refuses or rewrites a topic, copy the accepted topics back into the file. Unless step 11 failed, remove the scratch directory: `ROOT="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"; D="${TMPDIR:-/tmp}/repo-docs-$(printf %s "$ROOT" | cksum | cut -d' ' -f1)"; rm -rf "${D:?}"`. When another skill invoked this one, return to it and leave the commit to it; otherwise suggest `git-commit`, since these documents belong in one commit.
...
* **About panel in a file.** GitHub keeps the description, website, and topics only in its own settings. `.github/settings.yml` records them under version control in the Settings App format, the most common convention, so installing that app later syncs the file without a migration; drafting it with the README keeps the description in step with the tagline.
...
* NEVER write in the repository outside the in-scope documents, their translations in `docs/i18n/`, `NOTICE` and `COPYING` (when `references/licenses.md` calls for them), `.github/assets/`, and `.github/settings.yml`; downloads and saved originals stay in `$D`.
